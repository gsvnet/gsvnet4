...
class MemberController extends Controller
{
    ...
    public function updatePhoto(UpdatePhotoRequest $request, User $user)
    {
        ...
        $imageHandler->destroy($user->profile->photo_path);

        // Upload new photo. Path is generated by the system.
        // File cannot be serialized, so that's why we save it outside of the job.
        if (!$path = $imageHandler->store($request->file('photo_path')))
            throw new PhotoStorageException;

        SetProfilePicture::dispatch($user, $request->user(), $path);

...
<form method="GET" action="">
    <input
        ...
        id="naam"
        name="naam"
        placeholder="typ maar gewoon iets"
        value="{{ request()->input('naam') }}"
    >

    <select name="regio" id="regio">
        <option value="0">Maakt niet uit</option>
        @foreach ($regions as $region)
            <option value="{{ $region->id }}" {{ request()->input('regio') == $region->id ? 'selected' : '' }}>
                {{ $region->name }}
            </option>
        @endforeach
    </select>
    ...
    <select name="jaarverband" id="jaarverband">
        <option value="0">Doet er niet toe</option>
        @foreach ($yearGroups as $yearGroup)
            <option value="{{ $yearGroup->id }}" {{ request()->input('jaarverband') == $yearGroup->id ? 'selected' : '' }}>
                {{ $yearGroup }}
            </option>
        @endforeach
    </select>
    ...
    <label>
        <input
            ...
            name="oudleden"
            value="1"
            {{ request()->input('oudleden') == 1 ? 'checked' : '' }}
        />
        Oudleden weergeven
    </label>
